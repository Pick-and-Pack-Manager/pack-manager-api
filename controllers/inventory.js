...
router.get('/', async (req, res) => {
	console.log('Inventory Query connecting...')
      ...
      const query = `
			Select T0.ItemCode, T1.itmsGrpNam, T0.itemName, T2.WhsCode, T2.OnHand, Isnull(T2.StockValue,0) From OITM T0 inner join OITB T1 on T1.itmsGrpCod = T0.itmsGrpCod inner join OITW T2 on T2.ItemCode = T0.ItemCode where T2.OnHand <> 0 order by T2.WhsCode, T2.ItemCode
			`;
			// Execute the query
